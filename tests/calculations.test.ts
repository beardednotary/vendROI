...
function assertApprox(actual: number, expected: number, epsilon = 0.001, label = 'value'): void {
...
run('calculateDashboardMetrics computes ROI outputs', () => {
  const metrics = calculateDashboardMetrics(dashboard);
...
run('calculateProductMixMetrics aggregates products and identifies best performer', () => {
  const productMix: ProductMixData = {
    id: 'pm1',
    dashboardId: 'd1',
    products: [
  ...
  const metrics = calculateProductMixMetrics(productMix);
...
run('calculateGrowthSummary aggregates projection totals', () => {
  ...
  const summary = calculateGrowthSummary(projections);
...
run('dashboard calculations handle zero and negative values safely', () => {
  ...
  const zeroMetrics = calculateDashboardMetrics(zeroDashboard);
  ...
  const lossDashboard: DashboardData = {
    ...dashboard,
    revenue: {
      itemsSoldPerDay: 1,
      averageSalePrice: 1,
  ...
  const lossMetrics = calculateDashboardMetrics(lossDashboard);
...
run('product mix and growth summary handle empty arrays safely', () => {
  ...
  const mixMetrics = calculateProductMixMetrics(emptyMix);
  ...
  const growthMetrics = calculateGrowthSummary([]);
...
run('location metrics handle low traffic and zero dashboard sale inputs', () => {
  const lowTrafficLocation: Location = {
    id: 'l-low',
    name: 'Quiet Hallway',
    type: 'Office',
    footTraffic: 50,
  ...
  const dashboardWithZeroPrice: DashboardData = {
    ...dashboard,
    revenue: {
      itemsSoldPerDay: 0,
      averageSalePrice: 0,
  ...
  const metrics = calculateLocationMetrics(lowTrafficLocation, dashboardWithZeroPrice);
...
run('formatters and color helpers return stable outputs', () => {
  assert(formatCurrency(1234) === '$1,234', 'formatCurrency');
  assert(formatPercent(12.34) === '12.3%', 'formatPercent');
  assert(getROIColor(120) === '#00C853', 'getROIColor');
  assert(getProfitColor(-1) === '#FF5252', 'getProfitColor');
});
